...
def answer_paper_question(arxiv_id_or_url: str, question: str) -> dict:
    """
    Answers a question about an arXiv paper based on its PDF content.
    ...
    try:
        ...
        if not pdf_url:
            return {"status": "error", "message": "No PDF link found for this paper."}
        ...
        if not question_words:
            return {
                ...
                "content_sample": full_text[:1000],
                "title": paper.title,
            }

        # Basic keyword matching in full text
        found_keywords = 0
        ...
        if found_keywords > len(question_words) / 2:
            return {
                "status": "success",
                "answer_type": "found_in_pdf",
                "message": "The PDF may contain information relevant to your question. Please review the extracted content.",
            ...
            return {
                "status": "success",
                "answer_type": "not_found_in_pdf",
                "message": "I could not find specific information for your question in the paper's PDF.",
                "content_sample": full_text[:1000],
                "title": paper.title,
            }
...
if __name__ == "__main__":
    ...
    if (
        search_results_no_results["status"] == "success"
        and not search_results_no_results["papers"]
    ):
    ...
    question1 = (
        "What are the key results of this paper regarding large language models?"
    )
    answer1 = answer_paper_question(paper_id_for_qna, question1)
    print(f"\nQuestion 1: '{question1}'")
    print(f"Answer 1: {answer1['status']} - {answer1['message']}")
    if answer1["status"] == "success" and "content_sample" in answer1:
        print(f"Content sample: {answer1['content_sample'][:100]}...")
    ...
    ids_to_test = [
        ...
        "arxiv.org/pdf/2303.10130v1.pdf",
        "1234.56789",
        "cond-mat/0123456",
        "https://arxiv.org/abs/cond-mat/0123456v2",
        "This is my paper 2401.00001, check it out!",
        "No ID here",
    ...
    expected_ids = [
        ...
        "2401.00001",
        None,
        "1234.5678",  # Assuming the regex is greedy for the last one
    ]

    for i, id_str in enumerate(ids_to_test):
        extracted = _extract_arxiv_id(id_str)
        print(
            f'Input: "{id_str}" -> Extracted: "{extracted}", Expected: "{expected_ids[i]}" {"PASS" if extracted == expected_ids[i] else "FAIL"}'
        )
    ...
    print(
        f'Input: "{old_id_test}" -> Extracted: "{extracted_old_id}", Expected: "hep-th/0101001" {"PASS" if extracted_old_id == "hep-th/0101001" else "FAIL"}'
    )

    another_id = "cs.CV/0102003"
    extracted_another_id = _extract_arxiv_id(another_id)
    print(
        f'Input: "{another_id}" -> Extracted: "{extracted_another_id}", Expected: "cs.CV/0102003" {"PASS" if extracted_another_id == "cs.CV/0102003" else "FAIL"}'
    )

    id_with_version = "1706.03762v5"
    extracted_id_with_version = _extract_arxiv_id(id_with_version)
    print(
        f'Input: "{id_with_version}" -> Extracted: "{extracted_id_with_version}", Expected: "1706.03762v5" {"PASS" if extracted_id_with_version == "1706.03762v5" else "FAIL"}'
    )
